...
from BinActivateFunc import BinActivateFunc_bireal
...
class ORConv2d(Conv2d):
    ...
    def forward(self, input):
        binary_input = self.binfunc(input)
        binary_weight = self.binfunc(self.rotate_arf())
        a = torch.mean(torch.abs(self.rotate_arf()))
        return F.conv2d(binary_input, binary_weight, self.bias, self.stride,
                self.padding, self.dilation, self.groups)*a
